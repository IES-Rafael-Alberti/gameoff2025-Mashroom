...
@export_group("Basics")
@export var base_spawn_time: float = 1.0
@export var fish_eye: float = 0.0
@export var base_objects_speed: float = 2.0
...
var objects_speed: float = base_objects_speed
...
func _outside_spawns() -> void: # Obstacles that doesn't affect Gameplay
...
func _obstacle_spawns() -> void: # Obstacles for the gameplay
...
func _spawn_object_pos(object: PackedScene, pos_range: float) -> void: 
	# pos_range: Range between 0 and 100
	...
	next_object.global_position = spawn_pos + Vector2(
		(spawn_point.position.x - spawn_pos.x) / (fish_eye + 1),
		0,
	)
